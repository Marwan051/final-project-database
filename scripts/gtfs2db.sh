...
#
# This script loads raw GTFS data into staging tables.
# Use gtfs-etl.sh to transform staging data into operational schema.
#
# Usage:
#   ./gtfs2db.sh                    # Load into staging only
#   ./gtfs2db.sh --with-etl         # Load + run ETL transformation
#   DB_HOST=localhost ./gtfs2db.sh
#
...
#   PGPASSWORD (optional)
#   GTFS_DIR (default: /gtfs-data or ./gtfs-data)
...
GTFS_DIR="${GTFS_DIR:-$GTFS_DIR_DEFAULT}"
...
# NOTE: We do NOT truncate staging tables - they preserve historical raw data
# Each import updates existing records (ON CONFLICT) or adds new ones
# The imported_at timestamp tracks when each record was last loaded
...
echo
echo "==== GTFS STAGING import finished successfully ===="
